...
import { Vector3, PerspectiveCamera, OrthographicCamera, DoubleSide } from "three"
...
export const Html = React.forwardRef(
  (
    ...
    //EDIT
    var { size: Size, gl, camera, scene, raycaster, events, viewport } = useThree()
    var fov = camera.projectionMatrix.elements[5] * (Size.height / 2)
    ...
    const target = portal?.current || events.connected || gl.domElement.parentNode
    ...
    const transformInnerStyles = React.useMemo(() => ({ position: "absolute", pointerEvents }), [pointerEvents])
      ...
      if (group.current) {
        ...
        if (
          ...
          if (transform) {
            ...
            //EDIT
            if (oldZoom.current - camera.zoom !== 0) {
